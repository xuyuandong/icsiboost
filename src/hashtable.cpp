...
{
	hashtable_t* output = (hashtable_t*)MALLOC(sizeof(hashtable_t));
	output->buckets = (hashelement_t**)MALLOC(sizeof(hashelement_t*)*initial_capacity);
...
array_t* string_array_from_hashtable(hashtable_t* input)
{
	...
	while(element!=NULL)
	{
		array_push(output, string_new_from_to((const char*)element->key, 0, element->key_length));
		array_push(output, string_copy((string_t*)element->value));
...
int hashtable_save(hashtable_t* h,FILE* file, off_t (*saveValue)(hashelement_t* element,void* metadata), void* metadata)
{
	...
	size_t *bucketLength = (size_t*)MALLOC(sizeof(size_t)*h->size);
	for(i=0;i<h->size;i++)
	{
...
void hashtable_resize(hashtable_t* input, size_t new_size)
{
	...
	input->buckets = (hashelement_t**)MALLOC(sizeof(hashelement_t*)*new_size);
