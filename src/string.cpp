...
string_t* string_resize(string_t* input,size_t newSize)
{
	...
	input->data = (char*)REALLOC(input->data,newSize);
	input->size=newSize;
	return input;
}
...
#else
	output->data = (char*)MALLOC(length+1);
#endif
...
#else
	output->data = (char*)MALLOC(to-from+1);
#endif
...
regexstatus_t* _regexstatus_new(const char* pattern, const char* replacement, const char* flags_string)
{
	...
	if(flags_string!=NULL)
	{
		...
		while(current_flag && *current_flag!='\0')
		{
			switch(*current_flag) {
				...
				default:
					if(replacement==NULL) {
						die("In _regexstatus_new: invlid flag \"%c\", in m/%s/%s",*current_flag, pattern, flags_string);
					} else {
						die("In _regexstatus_new: invlid flag \"%c\", in s/%s/%s/%s",*current_flag, pattern, replacement, flags_string);
					}
					break;
			}
			current_flag++;
	...
	if((flags & REGEX_FLAG_ONCE) == 0)
	{
		...
		{
			status = (regexstatus_t*)hashtable_get(_regex_cache, pattern, strlen(pattern));
		}
	}
#endif
			...
			(flags & (REGEX_FLAG_CASE_INSENSITIVE|REGEX_FLAG_MULTILINE|REGEX_FLAG_NOSUB))))
	{
		...
		if(status->expression==NULL)
		{
			if(replacement==NULL)
			{ 
				die("In _regexstatus_new: compiling regex m/%s/%s, at char %d, %s", pattern, flags_string, error_offset, error_message);
			} else {
				die("In _regexstatus_new: compiling regex s/%s/%s/%s, at char %d, %s", pattern, replacement, flags_string, error_offset, error_message);
			}
		}
		if(pcre_fullinfo(status->expression, NULL, PCRE_INFO_CAPTURECOUNT, &status->num_subs)!=0)
			die("In _regexstatus_new: could not get number of groups in regex /%s/", pattern);
		status->num_subs++;
	}
...
array_t* string_array_grep(array_t* input, const char* pattern, const char* flags)
{
	...
	for(i=0;i<input->length;i++)
	{
		string_t* string = (string_t*)array_get(input,i);
...
int string_replace(string_t* input, const char* pattern, const char* replacement, const char* flags)
{
	...
	while(pcre_exec(status->expression,NULL,input->data,input->length,last_start,0,ovector,status->num_subs*3)>=0)
	{
		...
		{
			array_push(output,string_copy((string_t*)vector_get(status->between_replacements, i)));
			int32_t* id = (int32_t*)_vector_get(status->replacement_mapping, i);
			if(*id>0 && *id<status->num_subs)
				array_push(output,string_new_from_to(input->data, ovector[(*id)*2], ovector[(*id)*2+1]));
		}
		array_push(output,string_copy((string_t*)vector_get(status->between_replacements, status->between_replacements->length-1)));
		last_start=ovector[1];
		matched=1;
...
regexstatus_t* _regexstatus_new(const char* pattern, const char* replacement, const char* flags_string)
{
	...
	if(flags_string!=NULL)
	{
		...
		while(current_flag && *current_flag!='\0')
		{
			switch(*current_flag) {
				...
				default:
					if(replacement==NULL) {
						die("In _regexstatus_new2: invlid flag \"%c\", in m/%s/%s",*current_flag, pattern, flags_string);
					} else {
						die("In _regexstatus_new2: invlid flag \"%c\", in s/%s/%s/%s",*current_flag, pattern, replacement, flags_string);
					}
					break;
			}
			current_flag++;
			...
			(flags & (REGEX_FLAG_CASE_INSENSITIVE|REGEX_FLAG_MULTILINE|REGEX_FLAG_NOSUB))))
	{
		...
		if(regcomp_result!=0)
		{
			...
			if(replacement==NULL)
			{ 
				die("In _regexstatus_new2: compiling regex m/%s/%s, %s", pattern, flags_string, buffer);
			} else {
				die("In _regexstatus_new2: compiling regex s/%s/%s/%s, %s", pattern, replacement, flags_string, buffer);
			}
		}
		...
		while(*current!='\0')
		{
			if(*current=='$' && *(current+1)>='1' && *(current+1)<='9')
			{
				...
				else
				{
					...
					string_t* group_id=string_new_from_to(current, 1, last_current-current);
					int32_t id=string_to_int32(group_id);
					if(id<1 || id>status->expression.re_nsub)
                        warn("invalid group replacement \"$%s\", s/%s/%s/%s, %ld group%s", group_id->data, 
...
void string_array_fprintf(FILE* stream, const char* format, array_t* array)
{
	...
	{
		string_t* string = (string_t*)array_get(array, i);
